"""
Updated by Lin Xiong Sep-4, 2017
"""
...
def multi_factor_scheduler(begin_epoch, epoch_size, step=[20, 40, 60, 80, 90, 110], factor=0.1):
...
def main():
    ...
    model = mx.model.FeedForward(
        ...
        lr_scheduler        = multi_factor_scheduler(begin_epoch, epoch_size, step=[220, 260, 280], factor=0.1)
                             ...
                             multi_factor_scheduler(begin_epoch, epoch_size, step=[20, 40, 60, 80, 90, 110], factor=0.1),                                                        
        )
    model.fit(
        X                  = train,
        eval_data          = val,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="command for training xception network")
    ...
    hdlr = logging.FileHandler('./log/log-xecption-{}.log'.format(args.data_type))
